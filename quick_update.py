#!/usr/bin/env python3
"""
Optimized script to check for new Paul Graham essays.
This script will:
1. Check the most recent essay on Paul Graham's website
2. If it's already in our database, stop immediately
3. If not, only process the new essays until we find one we already have
"""
...
def main():
    ...
    print(f"Starting quick essay check at {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    
    # Get existing essays
    existing_essays = get_existing_essays()
    
    # Fetch the list of essays from Paul Graham's website
    essays = fetch_latest_essays()
    
    if not essays:
        print("Error: Could not fetch essays from Paul Graham's website")
        return
    
    # Check if the most recent essay is already in our database
    most_recent_essay = essays[0]  # Paul Graham's essays are listed with newest first
    
    if most_recent_essay['url'] in existing_essays:
        print(f"Most recent essay '{most_recent_essay['title']}' is already in our database")
        print("No new essays to add")
        return
    
    # If we get here, there are new essays to add
    print(f"Found new essays to add, starting with '{most_recent_essay['title']}'")
    
    # Process new essays until we find one we already have
    new_essays = []
    for essay in essays:
        ...
        print(f"Processing new essay: {essay['title']} ({essay['url']})")
    ...
    if new_essays:
        ...
        print("No new essays were added")
